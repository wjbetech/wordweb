 ...
 */
export function getThemeClasses(isDark: boolean, darkClasses: string, lightClasses: string): string {
...
export const themeClasses = {
  ...
  sidebarBg: (isDark: boolean) =>
    getThemeClasses(isDark, "bg-zinc-800", "bg-slate-50 border-slate-300 border shadow-lg"),

  panelBg: (isDark: boolean) =>
    getThemeClasses(isDark, "border-zinc-600 bg-zinc-900/30", "border-gray-200 bg-gray-50/50"),

  cardBg: (isDark: boolean) => getThemeClasses(isDark, "border-zinc-700 bg-zinc-800/50", "border-gray-200 bg-white/80"),

  // Content panel styling
  contentPanel: (isDark: boolean) =>
    getThemeClasses(isDark, "bg-zinc-900/30", "bg-gray-50/50") + " rounded-b-lg p-4 space-y-4 -mt-1", // No border, rounded bottom only, negative margin to merge with tabs

  settingsCard: (isDark: boolean) =>
    getThemeClasses(isDark, "border-zinc-700 bg-zinc-800/50", "border-gray-200 bg-white/80") +
    " border rounded-lg p-3 space-y-3",

  // Text variations
  primaryText: (isDark: boolean) => getThemeClasses(isDark, "text-gray-100", "text-slate-800"),

  secondaryText: (isDark: boolean) => getThemeClasses(isDark, "text-gray-300", "text-gray-700"),

  mutedText: (isDark: boolean) => getThemeClasses(isDark, "text-gray-400", "text-gray-600"),

  // Interactive elements
  hoverBg: (isDark: boolean) => getThemeClasses(isDark, "hover:bg-zinc-700", "hover:bg-gray-100"),

  buttonHover: (isDark: boolean) =>
    getThemeClasses(isDark, "text-white hover:bg-[#4c5c68]", "text-slate-700 hover:bg-slate-200"),

  // Borders
  border: (isDark: boolean) => getThemeClasses(isDark, "border-zinc-600", "border-gray-200"),

  borderSubtle: (isDark: boolean) => getThemeClasses(isDark, "border-zinc-700", "border-gray-200"),
  ...
  searchTag: (isDark: boolean) =>
    getThemeClasses(isDark, "bg-zinc-700 text-gray-200 hover:bg-zinc-600", "bg-zinc-600 text-white hover:bg-zinc-700") +
  ...
  searchTagRemove: (isDark: boolean) =>
    getThemeClasses(isDark, "text-gray-400 hover:text-red-400", "text-gray-300 hover:text-red-300") +
  ...
  errorButtonBorder: (isDark: boolean) => getThemeClasses(isDark, "border-red-600", "border-red-300"),
  ...
  dropdownMenu: (isDark: boolean) =>
    getThemeClasses(isDark, "bg-zinc-800 border-zinc-600 text-gray-200", "bg-white border-gray-300 text-gray-700") +
  ...
  toggleIcon: () => "block transform transition-transform duration-200 hover:scale-110"
};
