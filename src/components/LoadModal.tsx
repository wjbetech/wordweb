...
export default function LoadModal({ isOpen, isDark = false, saves, onClose, onLoad, onDelete }: LoadModalProps) {
    ...
    <div className="modal modal-open">
      <div className={`modal-box ${isDark ? "bg-zinc-800 text-white" : "bg-white text-gray-900"}`}>
        ...
        <div
          ...
          <span className="font-bold">Warning:</span> Loading a saved word web will{" "}
              ...
              {saves.map((s) => (
                <li
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <button className="btn btn-sm btn-secondary" onClick={() => onLoad(s.name)}>
                      Load
                    </button>
                    <button className="btn btn-sm btn-ghost hover:bg-error" onClick={() => onDelete(s.name)}>
                      Delete
                    </button>
                  </div>
                </li>
